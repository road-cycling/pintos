...
/* Transitions a blocked thread T to the ready-to-run state.
   ...
   update other data. */
   // Wakes up Sleeping & Blocked Threads
void
...
void thread_sleep(int64_t ticks) {
  ...
  if (nextWakeUpTime == -1) {
    nextWakeUpTime = ticks;
  } else if (nextWakeUpTime > ticks) {
    nextWakeUpTime = ticks;
  }

  list_insert_ordered(&sleep_queue, &cur->sleepElem, &sort_sleep, NULL);
  thread_block();
}

void on_timer_interrupt(int64_t accum_ticks) {
  if (accum_ticks >= nextWakeUpTime && nextWakeUpTime != -1) {
    struct thread *t = list_entry(list_pop_front(&sleep_queue), struct thread, sleepElem);
    thread_unblock(t);

    while (!list_empty(&sleep_queue)) {
      struct thread *a = list_entry(list_front(&sleep_queue), struct thread, sleepElem);
      if (accum_ticks >= a->wakeUpTime) {
        list_pop_front(&sleep_queue);
        thread_unblock(a);
      } else {
        nextWakeUpTime = a->wakeUpTime;
        break;
      }
    }
